...
as $$
  select
    ...
    coalesce(
      pc.config #>> '{selectedStartup,last30DaysRevenue}',
      pc.config #>> '{matchedStartup,last30DaysRevenue}'
    ) as last_30_days_revenue,
    coalesce(
  ...
  group by pc.user_id, startup_slug, mrr, last_30_days_revenue, all_time_revenue
